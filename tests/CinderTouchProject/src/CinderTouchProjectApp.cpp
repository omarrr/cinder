...
class CinderTouchProjectApp : public AppCocoaTouch {
  public:
	virtual void	setup();
...
void CinderTouchProjectApp::setup()
{
...
void CinderTouchProjectApp::resize( int width, int height )
{
	mCam.lookAt( Vec3f( 3, 2, -3 ), Vec3f::zero() );
	mCam.setPerspective( 60, width / (float)height, 1, 1000 );
}

void CinderTouchProjectApp::mouseDown( MouseEvent event )
{
	std::cout << "Mouse down @ " << event.getPos() << std::endl;
}

void CinderTouchProjectApp::update()
{
	mCubeRotation.rotate( Vec3f( 1, 1, 1 ), 0.03f );
}

void CinderTouchProjectApp::draw()
{
...
CINDER_APP_COCOA_TOUCH( CinderTouchProjectApp, RendererGl )
